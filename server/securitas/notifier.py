    ...
    def get(self):
        ...
        if (self.control.device is not None):
            ...
            if (uri.startswith('/control/alarm')):
                ...
                self.gcm.plaintext_request(registration_id=device,
                                           data=data)

            elif (uri == '/control/start'):
                print 'Start detection..'
                if (self.control.process is None):
                    process = subprocess.Popen(['motion', '-c',
                ...
                if (self.control.process is not None):
                    ...
                    try:
                        httpClient.fetch('http://localhost:8080/0/action/'
                                         + 'snapshot')
